<template>
  <view>
    ...
    <swiper :indicator-dots="true" :autoplay="true" :interval="3000" :duration="1000" :circular="true">
      ...
      <swiper-item v-for="(item, i) in swiperList" :key="i">
        <view class="swiper-item">
          <!-- 动态绑定图片的 src 属性 -->
          <image @click="goodList(item.goods_id)" :src="item.image_src"></image>
        </view>
      </swiper-item>
    </swiper>
    ...
    <!-- 分类导航区域 -->
    <view class="nav-list">
      <view @click="clickNavs(item)" class="nav-item" v-for="(item, i) in navList" :key="i">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goodList(id) {
        uni.navigateTo({
          url: '/subpkg/goods_list/goods_list' + id
        })
      },
      async Getfloors() {
      ...
      GoodList(url) {
        ...
        uni.navigateTo({
          url: '/subpkg/god_list/god_list?' + url.split('?')[1]
        })
      },
      clickNavs(item) {
        if (item.name === '分类') {
